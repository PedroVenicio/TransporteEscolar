...
def rota_ida_controller():
        if request.method == 'POST':
            try:
                ...
                print(data)
                rota_ida = Rota_ida(data['data'], data['hora'], data['alunos'])
            ...
            try:
                ...
                delete_rota_ida = Rota_ida.query.get(delete_rota_ida_id)
...
def rota_volta_controller():
        if request.method == 'POST':
            try:
                data = request.get_json()
                print(data)
                rota_ida = Rota_volta(data['data'], data['hora'], data['alunos'])
                db.session.add(rota_ida)
                db.session.commit()
            ...
            try:
                data = Rota_volta.query.all()
                lista = {'rotas_volta': [rota_ida.to_dict() for rota_ida in data]}
                return lista
            except Exception as e:
                return 'Não foi possível buscar rotas_ida. Error: {}'.format(str(e)), 405
            ...
            try:
                data = request.get_json()
                put_rota_ida_id = data['id']
                put_rota_ida = Rota_volta.query.get(put_rota_ida_id)
                if put_rota_ida is None:
            ...
            try:
                data = request.get_json()
                delete_rota_ida_id = data['id']
                delete_rota_ida = Rota_volta.query.get(delete_rota_ida_id)
                if delete_rota_ida is None:
                    return {'error': 'rota_volta não encontrada'}, 404
                db.session.delete(delete_rota_ida)
                db.session.commit()
